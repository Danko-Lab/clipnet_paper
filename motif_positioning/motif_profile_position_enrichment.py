...
@njit(parallel=True)
def permute(arr: np.ndarray, n: int = 10_000):
    zeros = np.zeros((n, arr.shape[0]))
    for i in prange(n):
...
permutations = permute(motif_positions, n=10_000)

permutations = [np.random.permutation(motif_positions) for _ in tqdm.trange(10_000)]
...
center_freq = (
    all_motifs.filter((all_motifs["distance_from_tss"] <= -40) & (all_motifs["distance_from_tss"] >= -100))
    .group_by("motif_prefix")
    .len()
    .rename({"len": "filtered_count"})
    .join(total_counts, on="motif_prefix")
...
dpe_freq = (
    all_motifs.filter(
        (all_motifs["distance_from_tss"] <= 30) & (all_motifs["distance_from_tss"] >= 20)
    )
    .group_by("motif_prefix")
    .len()
    .rename({"len": "filtered_count"})
...
pause_freq = (
    all_motifs.filter(
        (all_motifs["distance_from_tss"] <= 50) & (all_motifs["distance_from_tss"] >= 40)
    )
    .group_by("motif_prefix")
    .len()
    .rename({"len": "filtered_count"})
