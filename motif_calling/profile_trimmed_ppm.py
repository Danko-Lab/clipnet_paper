            ...
            if patterns_group_name in f.keys():
                ...
                if len(new_patterns_grp.keys()) > 0:
                    ...
                    for pattern in pattern_names:
                        ...
                        cwm_dict[patterns_group_name][pattern] = cwm  # / cwm.sum()
            ...
            if patterns_group_name in f.keys():
                ...
                if len(new_patterns_grp.keys()) > 0:
                    ...
                    for pattern in pattern_names:
                        ...
                        print(np.min(pass_inds), np.max(pass_inds) + 1)
...
cwms, cutoffs = load_motif_cwms(
    "all_tss_windows_reference_seq_profile_shap.modisco.h5"
)
...
motifs = {
    k: np.log2(ppms["pos_patterns"][f"pattern_{motif_idx_main[k]}"] / 0.25)
    for k in motif_idx_main.keys()
}
motif_cutoffs = {
    k: cutoffs["pos_patterns"][f"pattern_{motif_idx_main[k]}"]
    for k in motif_idx_main.keys()
}

attr = np.load("all_tss_windows_reference_seq_profile_shap.npz")["arr_0"]
...
ann = annotate_seqlets(X, seqlets, motifs, n_nearest=3, n_jobs=24)
...
filtered_seqlets = [
    ...
    for name in tqdm.tqdm(motif_idx_main.keys())
]
